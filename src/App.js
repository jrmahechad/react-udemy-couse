...
class App extends Component {
  ...
  inputChangeHandler = event => {
    ...
    const chars = event.target.value ? event.target.value.split('') : [];
    this.setState({
  ...
  render() {
    const charComponents = this.state.chars.map((c, i) => {
      return (
        <CharComponent char={c} deleteHandler={() => this.deleteHandler(i)} />
      );
    });
